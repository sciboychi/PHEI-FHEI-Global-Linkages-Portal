...
import { MOCK_PARTNERSHIPS } from './services/mockData';
...
export const partnershipService = {
  ...
  async resolveLinkageReview(id: string, status: PartnershipStatus, remarks: string): Promise<{ error: any }> {
    try {
      ...
      // Exact matching for schema.sql columns: 'verification_status' and 'REMARKS' (all caps)
      const { error: updateError } = await supabase
        .from('linkages')
        .update({ 
          verification_status: status,
          REMARKS: remarks || '' // Casing for schema.sql 'REMARKS' column
        })
        .eq('id', id);
